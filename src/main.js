...
import { customEach, arrIndex, copyObj } from "./tools";
import menu from "./menuElements";

// CREATE MENU UI
const mds = menu.initMain("THE SORCERER'S APPRENTICE");
const header = menu.initHeader(mds);
...
const template = menu.initTemplatePanel(mds);

// OPTIONS
// =======
const options = menu.initOptionsPanel(mds);
const compBtn = options.add("button", undefined, undefined, {
  name: "compBtn",
});
compBtn.text = "Create Comp";
const queueBtn = options.add("button", undefined, undefined, {
  name: "queueBtn",
});
queueBtn.text = "Queue In AME";
const renderBtn = options.add("button", undefined, undefined, {
  name: "renderBtn",
});
...
// STATUS
// ====
const statusPanel = menu.initStatusPanel(mds);

const progressBar = statusPanel.add("progressbar", undefined, 0, 100, {
  name: "progressBar",
});
progressBar.preferredSize.width = 480;

const status = statusPanel.add("statictext", undefined, undefined, {
  name: "status",
});
...
compBtn.onClick = function () {
  ...
  if (progressBar.value === 100) mds.close();
...
function populateTemplates(mainTab) {
  ...
  for (let u = 0; u < idArray.length; u++) {
    ...
    if (
      compArr.length > 0 &&
      findLayers(/^!T|^!I|^!V|^!C|^!G|^!F|^!A/g, compArr[0]).length > 0
    ) {
  ...
  for (let i = 0; i < templateFolders.length; i++) {
    ...
    mainTab[
      "t" + templateFolders[i].name + "_" + templateFolders[i].id
    ] = mainTab.add(
      ui.addTab(
        "t" + templateFolders[i].name + "_" + templateFolders[i].id,
        templateFolders[i].name
      )
    );
    folderArray.push({
...
function checkFolder(folder) {
  for (var i = 1; i <= folder.items.length; i++) {
    if (
      folder.items[i].typeName == "Composition" &&
      folder.items[i].name === folder.name
    ) {
      idArray.push({
        name: folder.name,
        id: folder.id,
...
function poplateTabs(templateName, mainTab) {
  ...
  var comp = libItemsInFolder(
    regSafe(templateName.name),
    compFolder,
    "Composition"
  )[0];
  ...
  mainTab["t" + templateName.name + "_" + templateName.id][
    "content_" + templateName.name
  ] = mainTab["t" + templateName.name + "_" + templateName.id].add(
    ui.addTabbedPannel("content_" + templateName.name)
  );
  ...
  for (var i = 0; i < arrayToLoad.length; i++) {
    var typeMatches = /^![A-Z][a-z]*\(.*\)/.test(arrayToLoad[i].name)
      ...
      tabObj = {
        T: {
          tab: "Text Input",
          func: ui.addTextGroup,
          inText: /!T/.test(varType)
            ? [
                regSafe(arrayToLoad[i].text.sourceText.value.text),
        ...
        I: {
          tab: "Image",
          func: ui.addMediaGroup,
          inText: ["", arrayToLoad[i].enabled],
        },
        V: {
          tab: "Videos",
          func: ui.addMediaGroup,
        ...
        C: { tab: "Colors", func: ui.addColorGroup, inText: "" },
        G: { tab: "Group" },
        F: {
          tab: "Fonts",
          func: ui.addTextGroup,
    ...
    var groupData = arrayToLoad[i].name
      .split(terminalReg)[1]
      .replace(/(^\s*)|(\s*$)/g, "");
    ...
    if (template[tabID] === undefined && arrIndex(typeOptions, "l") === -1)
      template[tabID] = template.add(ui.addTab(tabID, tabName));
    ...
    if (varType === "!C") {
      ...
      for (var u = 1; u <= arrayToLoad[i]("Effects").numProperties; u++) {
        ...
        template[tabID][camelize(gdata)] = template[tabID].add(
          tObj
            .func(
              camelize(gdata),
              gdata,
        ...
        template[tabID][camelize(gdata)].picker.onClick = function () {
          colorBtn(this);
        };
      }
      continue;
    ...
    if (varType === "!G") {
      if (arrIndex(typeOptions, "v") !== -1) {
        ...
        template[tabID][camelize(groupData)] = template[tabID].add(
          "checkbox",
          undefined,
          undefined,
          { name: "checkbox" }
    ...
    if (varType === "!T") {
      ...
      if (/\(.*\)/.test(typeHeader)) {
        ...
        if (
          fontStylesMaster[comp.id][camelize(styleName + " Style")] !==
            undefined &&
          template[fontStylesMaster[comp.id][camelize(styleName + " Style")]][
            camelize(styleName + " Style")
          ].txt.text === ""
        ) {
          template[fontStylesMaster[comp.id][camelize(styleName + " Style")]][
            camelize(styleName + " Style")
    ...
    if (varType === "!I" || varType === "!A" || varType === "!V") {
      ...
      template[tabID][camelize(groupData)].browse.onClick = function () {
        browserBtn(this);
      };
    }
  }
...
function camelize(str) {
  ...
  return str
    ...
    .replace(/(?:^\w|\b\w|\s+)/g, function (match, index) {
...
function colorBtn(inputFld) {
  inputFld.parent.txt.text = decToRgb(
    colorpicker(colorize(inputFld.parent.txt.text))
  );
}
...
function colorpicker(result_color) {
  ...
  var color_decimal =
    $.os.indexOf("Windows") !== -1
      ? $.colorPicker()
      : $.colorPicker("0x" + rgbToHex(decToRgb(result_color)));
  ...
  $.writeln(color_rgb);
  result_color = [color_rgb[0] / 255, color_rgb[1] / 255, color_rgb[2] / 255];
  ...
  for (var i = 1; i <= folderObj.items.length; i++) {
    if (iType !== "Folder" && folderObj.items[i].typeName === "Folder") {
      resultsArr = resultsArr.concat(
        libItemsInFolderRec(reg, folderObj.items[i], iType)
      );
    }
...
function addLinkedPrecomps(folderName, newFolder, composition) {
  ...
  function copyPrecompFolder(oldLocation, newLocation) {
    for (var i = 1; i <= oldLocation.items.length; i++) {
      ...
      if (oldLocation.items[i].typeName === "Composition") {
        ...
        var replaceLayer1 = findLayers(
          regSafe(">> " + newComp.name + " <<"),
          composition
        );
  ...
  for (var z = 0; z < newPrecomps.length; z++) {
    ...
    for (var u = 0; u < newPrecomps.length; u++) {
      var replaceCount = 1;
      if (u === z) continue;
      var replaceLayer = findLayers(
        regSafe(">> " + newPrecomps[u].name + " <<"),
        newPrecomps[z]
      );
      status.text =
        "checking for " + newPrecomps[u].name + " in " + newPrecomps[z].name;
      ...
      customEach(replaceLayer, function (preLayerRep) {
        ...
        status.text =
          "replaced " + replaceCount + " " + newPrecomps[u].name + " precomp";
        replaceCount++;
      });
    }
  }
...
The image layer "' +
  ...
  var templateChildren =
    template[templateName.name]["content_" + templateName.text].children;
  ...
  function colorCheck(colour) {
    if (
      colour === "" ||
      /^#{0,1}[0-9a-fA-F]{3,6} *$|[0-9]+, *[0-9]+, *[0-9]+ */g.test(colour)
    ) {
  ...
  function imageTest(imgT, fileT) {
    ...
    if (/[\\\/]/g.test(imgT.text)) {
      ...
      imgT.text !== "" &&
      libItemsReg(regSafe(imgT.text), "Footage").length === 0
    ) {
...
function importExternal(cfolder) {
  for (var i = 0; i < externalImageList.length; i++) {
    ...
    status.text =
      "Loading External File: " + path.match(/[^\/\\]+\.([A-z]+)/g)[0];
    ...
    function tryToLoad(loadPath) {
      ...
      if (io.canImportAs(ImportAsType.FOOTAGE)) {
        ...
        var windows = $.os.indexOf("Windows") !== -1,
          ...
          wSpace = windows ? " " : "\\ ",
          fileLocation =
            String(new File(path).parent).replace(/\%20/g, wSpace) + slash,
          scriptLocation = String(new File($.fileName).parent).replace(
            /\%20/g,
            wSpace
          );
        ...
        try {
          ...
          if (loadAttempt === 0) {
            ...
            try {
              loadAttempt = 1;
              read_file.copy(
                read_file.fsName.replace(/\.(gif|jpg|jpeg|tiff|png)$/i, ".bmp")
              );
              if (
                tryToLoad(
                  read_file.fsName.replace(
...
function setText(textLayer, comp, newText) {
  ...
  var boxSize = layerTextDoc.boxText
    ? {
        width: layerTextDoc.boxTextSize[0],
        height: layerTextDoc.boxTextSize[1],
      }
    : undefined;
  var rectSize = layerTextDoc.boxText
    ? textLayer.sourceRectAtTime(0, false)
    : undefined;
  var alignment = ["c", "c"];
  ...
  if (/\(.*\)/.test(textLayer.name)) {
    ...
    if (
      fontStyles[camelize(styleName + " Style")] !== undefined &&
      imageList[fontStyles[camelize(styleName + " Style")]][
        camelize(styleName + " Style")
      ].txt.text !== ""
    ) {
      layerTextDoc.font =
  ...
  if (layerTextDoc.boxText && boxSize.height < layerTextDoc.leading * 1.75) {
    ...
    var maxLines = Math.floor(
      layerTextDoc.boxTextSize[1] / layerTextDoc.leading
    );
    ...
    if (
      alignment[0] === "c" &&
      textLayer.sourceRectAtTime(0, false).height < rectSize.height
    ) {
      //Center the text if it's not the full height and the anchor is the center
      var layerPosition = textLayer.position.value;
      var adjust =
        (rectSize.height - textLayer.sourceRectAtTime(0, false).height) / 2;
...
function anchorPoint(layer, bound) {
  ...
  if (bound === "box") {
    if (
      ...
      layer.sourceText.value.boxTextPos[1] +
        layer.sourceText.value.boxTextSize[1] / 4
    ) {
      cor[0] = "t";
    } else if (
      layer.anchorPoint.value[1] >=
      layer.sourceText.value.boxTextPos[1] +
        (3 * layer.sourceText.value.boxTextSize[1]) / 4
    ) {
      cor[0] = "b";
    }
...
function sendtoRender(composition, renderOp) {
  ...
  // Requires Adobe Media Encoder 11.0.
  {
    if (app.project.renderQueue.canQueueInAME == true) {
      ...
      app.project.renderQueue.queueInAME(
        renderOp === "queueOnly" ? false : true
      );
    } else {
...
function mdsRender(templateChoice, renderOp) {
  try {
    imageList = copyObj(
      template[templateChoice.name]["content_" + templateChoice.text]
    );
  } catch (e) {
    status.text = e;
  }

  var ORcompFolder = libItemsReg(
    Number(templateChoice.name.match(/[0-9]+$/g)[0]),
    "Folder",
    1
  );
  status.text = "Found Comp Folder";
  var ORcomp = libItemsInFolder(
  ...
  if (userComps == undefined)
    userComps = app.project.items.addFolder("User Comps");

  status.text = "Found or Created 'User Comps' Folder";

  //delete existing folder if needed
  if (
    libItemsInFolder("^" + regSafe(comp.name) + "$", userComps, "Folder")
      .length > 0
  ) {
    var matchList = libItemsInFolder(
      "^" + regSafe(comp.name) + "$",
      userComps,
      "Folder"
    );
  ...
  function relinkExp(layr) {
    for (var i = 1; i <= layr.property("Effects").numProperties; i++) {
      ...
      if (matchName == "ADBE Fill" || matchName == "ADBE Color Control") {
        if (
          layr.property("Effects").property(i).property("Color")
            .expressionEnabled
        ) {
          var orExp = layr.property("Effects").property(i).property("Color")
              .expression,
          ...
          if (expressionComp === ORcomp.name) {
            ...
            customEach(preComps, function (item) {
              if (expressionComp === item.name) {
                ...
                expressionComp === item.name.replace("[" + comp.name + "] ", "")
              ) {
                newExp = orExp.replace(exReg, item.name);
              }
            });
          ...
          layr
            .property("Effects")
            .property(i)
            .property("Color").expression = newExp;
        }
      }
    }
  }
...
function fillTemplate(comp, compFolder, templateChoice, renderOp) {
  ...
  function replaceContent(layer) {
    ...
    if ((varType === "!I" || varType === "!V") && layerField.img.text !== "") {
      ...
      layer.replaceSource(
        libItemsReg(regSafe(layerField.img.text), "Footage", 1),
        false
      );

      if (layer.width / layer.height <= orSize.width / orSize.height) {
        if (
          arrIndex(typeOptions, "f") == -1 &&
          arrIndex(typeOptions, "b") == -1
        ) {
      ...
      if (
        arrIndex(typeOptions, "s") !== -1 &&
        layer.transform.scale.value[0] > 100
      ) {
      ...
      if (arrIndex(typeOptions, "f") !== -1) {
        var newMask = layer.Masks.addProperty("Mask"),
          ...
          bounds = [
            ((layer.width * sca[0] * 0.01 - orSize.width) * orPer[0]) /
              (sca[0] * 0.01),
            ((layer.height * sca[1] * 0.01 - orSize.height) * orPer[1]) /
              (sca[1] * 0.01),
            ((layer.width * sca[0] * 0.01 - orSize.width) * orPer[0]) /
              (sca[0] * 0.01) +
    ...
    if (varType === "!A" && layerField.audio.text !== "") {
      layer.replaceSource(
        libItemsReg(regSafe(layerField.audio.text), "Footage", 1),
        false
      );
    }

    //Check to see if the layer needs to be turned on or off
    if (layerField.visCheck !== undefined)
      layer.enabled = Boolean(layerField.visCheck.value);
